...
def has_cask_installed():
    '''
    Check if `brew cask` is available.
    '''
    user = __salt__['file.get_user'](_homebrew_bin())
    return not bool(__salt__['cmd.retcode']('brew cask', runas=user))
...
def install(name=None, pkgs=None, taps=None, options=None, **kwargs):
    ...
    if 'No available formula' in install_ret and has_cask_installed():
        # try installing via cask
        ret = __salt__['cmd.run'](
            cmd.replace('brew', 'brew cask'),
            runas=user if user != __opts__['user'] else None,
            python_shell=False,
            output_loglevel='trace'
        )
    ...
    __context__.pop('pkg.list_pkgs', None)
    new = list_pkgs()
    egg = salt.utils.compare_dicts(old, new)
    log.error('{}: {}'.format(name, egg))
    return egg
